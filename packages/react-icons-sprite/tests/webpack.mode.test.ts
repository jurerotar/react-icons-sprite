...
describe('Webpack integration - mode check', () => {
  ...
  it('loader does NOT transform in development mode', async () => {
    const source = "import { BiAlarm } from 'react-icons/bi'; export const C = () => <BiAlarm/>;";
    const context = {
      mode: 'development',
      resourcePath: 'test.tsx',
    };

    const result = await reactIconsSpriteLoader.call(context as any, source, null as any);
